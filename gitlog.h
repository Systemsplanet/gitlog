...
// Maximum message length (ESP-NOW max payload is 250 bytes; reserve 1 for null terminator)
#define GITLOG_MAX_MSG_LEN 243

// Shared structures for ESP-NOW payload
typedef struct { char event_msg[GITLOG_MAX_MSG_LEN + 1]; } gitlog_message_t;
...
public:
  ...
  // Returns false if message is too long (will NOT silently truncate)
  bool broadcast(const char* message) {
    size_t msgLen = strlen(message);

    if (msgLen > GITLOG_MAX_MSG_LEN) {
      Serial.printf("ERROR: Message length %d exceeds maximum of %d chars. Broadcast aborted.\n",
                    (int)msgLen, GITLOG_MAX_MSG_LEN);
      return false;
    }

    gitlog_message_t msg;
    memset(&msg, 0, sizeof(msg));
    strncpy(msg.event_msg, message, GITLOG_MAX_MSG_LEN);
    msg.event_msg[GITLOG_MAX_MSG_LEN] = '\0';

    // Only send actual content length + null terminator (saves airtime)
    size_t sendLen = msgLen + 1;
    ...
    for (int attempt = 1; attempt <= MAX_RETRIES; attempt++) {
      ...
      for (uint8_t ch = 1; ch <= 11; ch++) {
        ...
        ackReceived = false;
        esp_now_send(serverAddress, (uint8_t *)&msg, sendLen);
...
private:
  ...
  // Pending upload state — set in ESP-NOW callback, processed from loop()
  volatile bool pendingUpload = false;
  char pendingGuid[13];
  char pendingMsg[GITLOG_MAX_MSG_LEN + 1];
  ...
  void pushToGitHub(String guid, String event) {
    ...
    client.setInsecure(); // NOTE: For production, replace with pinned GitHub root CA
    HTTPClient http;
  ...
  static void staticOnDataRecv(const uint8_t * mac, const uint8_t *incomingData, int len) {
    ...
    // Accept variable-length packets: at least 1 byte, at most full struct size
    if (len < 1 || len > (int)sizeof(gitlog_message_t)) return;
    ...
    esp_now_del_peer(mac); // Cleanup to prevent hitting max peer limit

    // 2. PROCESS PAYLOAD — copy to pending buffer, upload handled in loop()
    //    (Never do blocking I/O inside the ESP-NOW callback)
    if (instance->pendingUpload) {
      // Already have a pending upload; drop this packet to avoid overwrite
      Serial.println("WARNING: Dropped packet — previous upload still pending.");
      return;
    }
    ...
    msg.event_msg[GITLOG_MAX_MSG_LEN] = '\0'; // Ensure null termination

    snprintf(instance->pendingGuid, sizeof(instance->pendingGuid),
             "%02X%02X%02X%02X%02X%02X",
             mac[0], mac[1], mac[2], mac[3], mac[4], mac[5]);

    strncpy(instance->pendingMsg, msg.event_msg, GITLOG_MAX_MSG_LEN);
    instance->pendingMsg[GITLOG_MAX_MSG_LEN] = '\0';

    instance->pendingUpload = true; // Signal loop() to process
  }
...
public:
  GitlogServer(String token, String user, String repo, const char* ssid, const char* pwd) {
    instance = this;
    pendingUpload = false;
    memset(pendingGuid, 0, sizeof(pendingGuid));
  ...
  // Call this from loop() in server.ino
  void process() {
    if (pendingUpload) {
      pushToGitHub(String(pendingGuid), String(pendingMsg));
      pendingUpload = false;
    }
  }
};
